...
# Input from Day 4, Problem 1 of Advent of Code
...
for line in range(len(rawdata)):
    ...
    while len(calcsum) < 5:            # <-- While we still have less than 5 chars in our calcsum
        ...
        print(x)
        temp = ''                              # <-- Get read to add somethng to calcsum
        ...
        calcsum += ''.join(sorted(temp))  # Add to calcsum the sorted list of letters with x frequency
        print(calcsum)
